...
namespace Service_ClientDAO
{
    ...
    {
        public static void StartWorkWith(string SettingsFile)
        {
            Service.StartWorkWith(SettingsFile);
        }
                ...
                try
                {
                    ...
                    for (int j = 0; j < p.Count.Count; j++)
                    {
                        if (p.Count[j] >= productsCounts[i])
                        {
                            ...
                            Console.WriteLine(p.ShopID[j]);
                            if ( productsCounts.Count() == ShopListID.Where(item => item == p.ShopID[j]).Count() )
                            {
                                int temp = ShopListID[ShopListID.Count() - 1];
