...
namespace Service_ClientDAO
{
    public class ProductCSV : IProduct
    {
        ...
        public void UpdateProduct(string Name, int ShopID)
        {
            this.Connect();
            bool flag = true;
            foreach (var p in Products)
            {
                if (p.Name == Name)
                {
                    ...
                    {
                        int poss = p.ShopID.IndexOf(ShopID);
                        Console.WriteLine("Новое количество товара:");
                        p.Count[poss] = Convert.ToInt32(Console.ReadLine());
                        Console.WriteLine("Новая цена товара:");
                        p.Cost[poss] = Convert.ToDouble(Console.ReadLine());
                    }
                    else
                    {
                        p.ShopID.Add(ShopID);
                        ...
                        Console.WriteLine("Установить количество для товара:");
                        p.Count.Add(Convert.ToInt32(Console.ReadLine()));
                        Console.WriteLine("Установить количество для товара:");
                        p.Cost.Add(Convert.ToDouble(Console.ReadLine()));
                    }
                }
            }
            if (flag)
